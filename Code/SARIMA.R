...
plot_z
ggsave("Final Report/Plots/Ztplot.png", plot_z, width = 900, height = 350, units = 'px', scale = 2.5)

# adf test
adf.test(Z, k = 24)

png('Final Report/Plots/acfZ.png', height = 300, width = 600)
Acf(Z)
dev.off()
png('Final Report/Plots/pacfZ.png', height = 300, width = 600)
Pacf(Z)
dev.off()
...
fittedarima <- ggplot(dat.armodel, aes(x = Year, y=Revenue, col = Series)) +
    geom_line() + theme_light() + labs(title = "SARIMA(4,1,1)(1,0,2)[12]",
    ...
    xlab("Time (years)")
ggsave("Final Report/Plots/fittedarimaplot.png", fittedarima, width = 900, height = 350, units = 'px', scale = 3)
